...
Some Languages, e.g. Ada and Julia, provide explicit support for
...
- vector constructors
      - literals look like lists except with added =#= in front of the open parenthesis
            - =#(elem1 elem2 ... elemN)=
      - =vector= and =make-vector= constructor procedures
...
numbers, addresses and structures. Elements which require more than a machine
word are usually replaced with a pointer so they can be stored as a separate
object /- these low-level considerations are transparent to the programmer/.
...
contains two pointer fields, conventionally called =car= and =cdr=, aka =head=
...
Expert program designers often put a lot of work into storing data in such a way
that access and updating of the data will be very fast. This often leads to
complex code which can hide bugs. Its actually quite difficult to predict in
advance what will be the most efficient storage layout for data. The best
practice is to program for clarity rather than efficiency. Once the program has
all of its desired features, if it runs too slowly there are tools called
/profilers/ that can help the programmer find the performance bottlenecks, i.e.
the (usually very small) parts of the code which need to be sped up.

Programmers following best practice will redesign only the parts of the program
that are performance bottlenecks with a more efficient data structure (the way
storage is arranged in memory) or algorithm (the strategy of the program's
procedures). This way the overall complexity of the program is minimized.

Analyzing the time and space costs of algorithms is part of the discipline of
Computer Science. Most programmers have only a slight acquaintance with it and
tend to just trust that the more knowledgeable implementors of the language and
libraries which they're using will make their programs reasonably efficient.
Alas, this abrogation of responsibility is one of the reasons why modern
programs tend to be noticeably inefficient, e.g. annoyingly or horribly slow.
It's at least a good idea to consult a suitably experienced computer scientist
at strategic points in a software development project!
