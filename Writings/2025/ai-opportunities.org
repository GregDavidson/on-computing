...
- ExKAI :: Explicit Knowledge-based Artificial Intelligence.
...
- ImKAI systems typically require more resources to run than ExKAI and
  non-AI systems require
...
- Desired Output: Raster Graphic Image corresponding to the
  description which meets the user's needs.
...
- Good Application: Providing transcriptions to the /speaker/ for
  review and correction before being released to others!
...
- Training Data:
    - Massive amounts of SVG imagery accompanied by descriptions
        - available by scraping the web
- Input: Description of desired imagery
- Output: SVG Model of Graphic Image with objects and parts separated.
- Note: Good SVG Models are much more editable than Raster Graphic
  Images.  AI should be able to update parts without having to
...
- Input: Short description of desired new problem to solve
    - specifying desired formats, resources, etc.
- Output: Human-Understandable code using desired programming
  language(s), libraries, techniques, etc.
- State of the Art:
    - Output code often contain every kind of error.
...
3. Where possible, rather than directly performing a task
    - producing transparent, verifiable non-AI computer programs to do
      a task
    - producing explicit Knowledge-Based AI Systems where conventional
      programs can't do a task.
    - using LLM and similar non-explicit AI to support rather than
      replace the more transparent (auditable) approaches.
...
- Specifications
    - a /declarative/ model of the desired system /viewed from the outside/
        ...
        - it is declarative (specifying what is true in the problem domain)
        - not procedural (an algorithm, i.e. what to do, how it works)
- Architecture Model
    - Divides the system's functionality into modules
        ...
        - modules developed just as whole system developed
            - Declarative specification of module from the outside
            - Declarative specification of how modules collaborate
        - good modules exhibit
            - high-cohesion & low coupling
            - clean interfaces hide internal complexity
...
- Design Model and Implementation Model for each Module
    - The Design Model specifies higher-level layers
        ...
        - using high-level computing concepts, e.g.
            - generic operations, generic datatypes
    ...
    - The Implementation Model specifies lower-level layers
        ...
        - with specified mechanisms, e.g.
            - detailed algorithms and data types
            - procedures with complete signatures and computable bodies
            - specific types and other constraints
            - indexes and other performance-related elements (optional)
...
1. AI should almost always be used to more efficiently build non-AI
   systems.
